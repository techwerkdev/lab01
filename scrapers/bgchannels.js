...
const BU_CHANNELS_FILE_PATH = path.resolve(__dirname, '../sources/bgchannels.json');
const M3U_FILE_PATH = path.resolve(__dirname, '../output m3u/network_requestsBG.m3u');
...
async function processBGChannels() {
  ...
  try {
    ...
    // ✅ Пробај да кликнеш на SVG селектор ако постои
    const selector = '#player > div:nth-child(2) > div.container > div.player-poster.clickable';
    try {
      await page.waitForSelector(selector, { timeout: 5000 });
      await page.click(selector);
      console.log(`🖱️ Clicked on selector: ${selector}`);
    } catch (err) {
      console.warn(`⚠️ Selector not found or not clickable: ${selector}`);
    }
